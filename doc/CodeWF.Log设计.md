...
本文是 CodeWF.Log 的唯一设计基线，合并原“核心设计”和“Microsoft.Extensions.Logging 适配设计”。README 负责快速接入；本文负责事件模型、Pipeline、MEL、Avalonia、通知、Demo 与发布验收的完整约束。
...
7. 普通 `ILogger` 日志与 `LogUser*` 日志都生成同一种 `CodeWFLogEvent`；`LogUser*` 只额外提供可选的 `UserMessage`，`LogUserNotification(...)` 再显式设置通知请求。
...
正式 API 同时支持 `appsettings.json` 和 `AddCodeWF(options => ...)` 代码配置，并遵循 .NET Options 的正常合并顺序。WebApiDemo 为了集中验证配置绑定，故意只使用 `appsettings.json`；这不表示产品 API 不支持代码配置。
...
Demo 的原则是职责单一、界面低负担：不提供手工模板编辑器，只提供两档模板下拉框和少量组合按钮；高级模板语法由 README 和本文说明。日志消息模板固定，动态值使用结构化参数。
...
- 同一个 `ILogger<T>` 同时注册 Serilog 与 CodeWF Provider。
- Serilog 负责文件/控制台；CodeWF 禁用 File/Console，只负责 EventFeed、LogView 和通知。
- 提供 CodeWF LineTemplate 两档切换；Serilog 文件格式保持独立。
- 组合按钮覆盖普通日志、Message/UserMessage、EventId、Scope、Activity、显式通知和并发写入。
- Serilog 不接收 CodeWF 私有 UserMessage 属性，只接收标准诊断 State 和 `{OriginalFormat}`。

### 16.5 WebApiDemo

- 使用 `WebApplication.CreateBuilder(args)` 和 `builder.Logging.AddCodeWF()`。
- File、Console、EventFeed、Capture 和 Queue 配置只写在 `appsettings.json`，集中验证配置绑定。
- 展示 MEL 过滤、EventId、结构化参数、Scope、Activity、LoggerMessage 和 `/recent-logs`。
- `/recent-logs` 只用于 Demo；生产接口不得无鉴权暴露完整日志事件。
...
- FileNotifyDemo 验证不创建 LogView 时，文件输出和 DesktopWindow 重要通知仍可独立工作。
...
- 五个示例统一命名为 ConsoleDemo、FileNotifyDemo、LogViewDemo、SerilogDemo、WebApiDemo。
