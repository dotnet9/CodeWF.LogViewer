...
namespace AvaloniaLog4Net.Views
{
    public partial class MainWindow : Window
    {
        ...
        private void AddInfoLog_OnClick(object? sender, RoutedEventArgs e)
        {
            ...
            Logger.Info($"操作B - 用户ID：{Guid.NewGuid()}，执行操作：{GetRandomAction()}");
        }

        private void AddWarnLog_OnClick(object? sender, RoutedEventArgs e)
        {
            Logger.Warn($"数据库 - 查询[用户]时发现重复记录ID: {Random.Shared.Next(10000, 99999)}");
        }

        private void AddErrorLog_OnClick(object? sender, RoutedEventArgs e)
        {
            Logger.Error($"[接口调用] - 访问[订单接口]时发生异常信息：{new Exception("服务器错误").Message}", friendlyContent: "[友好日志，只显示在UI上]订单接口不正确，请联系管理员");
        }
        ...
        private void StartPerformanceTest_OnClick(object? sender, RoutedEventArgs e)
        {
            if (sender is Button button)
            {
                ...
                else
                {
                    ...
                    button.Content = "停止测试";
                    Logger.Info("性能测试已开始，将定时快速调用不同级别的日志接口。");
                }
            }
        }
        ...
        private void PerformanceTimer_Elapsed(object? sender, ElapsedEventArgs e)
        {
            ...
            Dispatcher.UIThread.Post(() =>
            {
                for (int i = 0; i < logsToWrite; i++)
                {
                    ...
                    {
                        case 0:
                            Logger.Debug($"性能测试[Debug] - 批次计数: {batchCounter}");
                            break;
                        case 1:
                            Logger.Info($"性能测试[Info] - 批次计数: {batchCounter}");
                            break;
                        case 2:
                            Logger.Warn($"性能测试[Warn] - 批次计数: {batchCounter}");
                            break;
                        case 3:
                            Logger.Error($"性能测试[Error] - 批次计数: {batchCounter}");
                            break;
                        case 4:
                            Logger.Fatal($"性能测试[Fatal] - 批次计数: {batchCounter}");
                            break;
                    }
                }
                ...
                Logger.Info($"性能测试 - 批次完成，本次写入 {logsToWrite} 条日志，累计写入 {_logCounter} 条日志。");
            });
        }
    }
