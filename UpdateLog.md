...
## 12.1.3.10 (2026-09-22)

- 🔒[加固]-将自定义日志级别文本纳入入队前事件快照预算，避免 `LocalizedLevelFormatter` 返回超长内容时绕过单事件容量限制。

## 12.1.3.9 (2026-09-22)

- 🔨[工程化]-移除 .NET 11 共享框架已提供的冗余 Microsoft.Extensions 包引用，保留 .NET 8 和 .NET 10 的显式依赖，清理目标框架专属还原警告。

## 12.1.3.8 (2026-09-22)

- 🔨[工程化]-同步文档中的 .NET 支持矩阵，保留 `net8.0`、`net10.0` 和 `net11.0`，并增加三目标框架构建、测试、打包和 Native AOT 产物执行的 CI 流程。

## 12.1.3.7 (2026-09-22)

- 🐛[修复]-补齐 `LogUser*` 消息模板的对齐和当前文化格式化语义，减少与标准 MEL 日志格式化结果的差异。

## 12.1.3.6 (2026-09-22)

- 🐛[修复]-通知宿主暂不可用时保留待展示日志并延迟重试，避免应用启动或窗口切换期间丢失显式通知。

## 12.1.3.5 (2026-09-22)

- 🐛[修复]-补齐关闭期间和 Channel 已关闭时的日志丢弃健康统计，避免 `CodeWFLogHealth` 低估实际丢弃数量。

## 12.1.3.4 (2026-09-22)

- 🔨[优化]-文件日志文件名加入进程 ID，轮转、保留和目录容量清理隔离到当前进程，降低多进程共享目录时的写入与删除竞争。

## 12.1.3.3 (2026-09-22)

- 🔒[加固]-日志进入后台队列前执行深层快照和单事件容量限制，避免外部可变集合、超大结构化值和异常对象图长期进入 Feed 与 Sink。

## 12.1.3.2 (2026-09-22)

- 🔨[优化]-为 LogEventFeed 和 Avalonia LogView 增加有界订阅队列，慢订阅者超限时丢弃最旧事件并提供丢弃计数，订阅者异常进入 Self Diagnostics。

## 12.1.3.1 (2026-09-22)

- 🐛[修复]-改进多个 `CodeWF` Provider 启用静态桥接时的生命周期管理，当前桥接释放后自动切换到下一个存活 Provider，并增加回归测试。

## 12.1.3.0 (2026-09-22)

- 🐛[修复]-修正 Native AOT smoke 项目将 `net11.0` 与 .NET 10 runtime 混用的问题，发布后实际运行验证恢复正常。
...
- 🔨[优化]-升级 Xaml.Behaviors 到 12.0.7。

## 12.1.0.28 (2026-07-29)

- 🚄[性能]-LogView 改为增量追加日志并按条目移除过期 Inline，只在模板、筛选或时间格式变化时全量重建。
- 🚄[性能]-LineTemplate 和 OutputTemplate 在校验时预编译并弱引用缓存，格式化阶段不再重复扫描和切分模板。
- ✅[测试]-增加模板并发复用、增量追加、头部淘汰和模板重建回归测试。

## 12.1.0.27 (2026-07-29)

- 🐛[修复]-默认 `LineTemplate` 的日志级别改用全角中括号 `【】`，与 LogView 既有视觉规范保持一致。
- 🔨[优化]-同步更新 LineTemplate 示例和演示预设，文件日志的独立 `OutputTemplate` 保持不变。

## 12.1.0.26 (2026-07-29)

- 🐛[修复]-LogView 恢复分段日志颜色：时间使用 `#8C8C8C`，仅级别及其括号使用级别色，描述使用 `#262626`。
- 🔨[优化]-分段渲染继续遵循运行时 `LineTemplate`，模板字段顺序、格式、增删和转义保持不变。

## 12.1.0.25 (2026-07-29)

- 🐛[修复]-桌面通知的级别背景限制在图标与级别文字区域，避免背景延伸到日志描述。
- 🔨[优化]-级别区域高度提取为主题资源，调用方可按需覆盖。

## 12.1.0.23 (2026-07-27)

- 🐛[修复]-“新日志”提示按设计稿补齐 73×16 标签、1px `#EFDBFF` 边框及居中的 55×12 文字区域。
- 🔨[优化]-桌面通知默认增加 1px `#D9D9D9` 窗体边框，在纯白背景上仍能清晰识别窗口边界；边框颜色和厚度继续支持资源覆盖。

## 12.1.0.22 (2026-07-27)

- 🐛[修复]-翻页按钮不再使用主题 `disabled` 伪类渲染，改由命中测试和四向状态图控制，避免 Fluent/Semi 降低透明度后禁用箭头消失。
...
- 📚[文档]-README 按 Console、LogView、无 LogView、Serilog、Web API 的顺序明确五种典型使用示例及依赖边界。
